...
const CAP_FIELDS: {
  key: keyof Omit<RoleOption, "code" | "label">;
  label: string;
  hint: string;
}[] = [
  {
  ...
  },
  {
    ...
    hint: "Mời/sửa/xoá tài khoản nhân viên.",
  },
  {
    key: "canManageContent",
    label: "Đăng nội dung website",
    hint: "Đăng/sửa/xoá Tin tức, Tuyển dụng, Sản phẩm.",
  },
  {
    key: "canDraftContent",
    label: "Lưu bản nháp nội dung",
    hint: "Tạo/sửa bản nháp nội dung của chính mình.",
  },
  {
    key: "canAccessLark",
    label: "Sử dụng Tạo file Lark",
    hint: "Tạo file, duyệt Drive, quản lý file Lark của chính mình tạo ra.",
  },
  {
    key: "canManageLarkOrgWide",
    label: "Quản lý mọi file Lark",
    hint: "Xoá/di chuyển/chuyển quyền sở hữu file Lark của bất kỳ ai.",
  },
  {
    key: "canViewLarkStats",
    label: "Xem thống kê Lark",
    hint: "Xem tab Thống kê và số liệu toàn công ty trong Tổng quan Lark.",
  },
  {
    key: "canViewInbox",
    label: "Xem hộp thư liên hệ",
    hint: "Xem form liên hệ khách hàng gửi từ website.",
  },
];
...
function CapFields({ defaults }: { defaults?: RoleOption }) {
  const [caps, setCaps] = useState<Record<string, boolean>>(
    Object.fromEntries(
      CAP_FIELDS.map((f) => [f.key, defaults?.[f.key] ?? false]),
    ),
  );
  return (
      ...
      {CAP_FIELDS.map((f) => (
        ...
        >
          <input
            ...
            checked={caps[f.key]}
            onChange={(e) =>
              setCaps((prev) => ({ ...prev, [f.key]: e.target.checked }))
            }
...
function RoleRow({ role }: { role: RoleOption }) {
  ...
  const [removeState, setRemoveState] =
    useState<RoleMutationState>(initialState);
  ...
  if (editing) {
    return (
      <form
        action={updateAction}
        className="flex flex-col gap-3 rounded-xl border border-accent/40 bg-wash/40 p-3.5"
      >
    ...
    <div className="flex items-start justify-between gap-3 rounded-xl border border-line px-3.5 py-3">
      <div className="flex min-w-0 flex-col gap-1">
        <span className="flex items-center gap-2 text-[13.5px] font-medium text-ink">
          <span className="rounded-full bg-wash px-2 py-0.5 text-[11px] font-semibold text-ink-2">
            {role.code}
          </span>
          {role.label}
        </span>
