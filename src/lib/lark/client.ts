...
export function getLarkApps(): LarkAppConfig[] {
  ...
  try {
    orgFolderTokens = process.env.LARK_ORG_FOLDER_TOKENS
      ? JSON.parse(process.env.LARK_ORG_FOLDER_TOKENS)
      : undefined;
  } catch {
...
// Every new file's actual storage location — deliberately NOT whichever app
// the creator's own lark_prefs.activeApp happens to be. The sub-orgs
// (SISMO/SAIZA/2S-CTV/ZENSIP) are on Lark's Free tier with a small storage
// cap; 2sgroup is the paid Pro-tier tenant. Centralizing every created file
// there, regardless of which org actually created it, keeps the free-tier
// tenants from filling up. The creator still gets full_access (or real
// ownership, if requested) on that file inside 2sgroup's Drive — see
// createLarkDocument's sharing step.
export function getStorageAppKey(): string {
  return "2sgroup";
}
...
async function getMySpaceRootFolderToken(appKey?: string): Promise<string> {
  ...
  const res = await fetch(
    `${LARK_API_BASE}/drive/explorer/v2/root_folder/meta`,
    {
  ...
  if (!res.ok || data.code !== 0) {
    throw new Error(
      `Không lấy được thư mục gốc của app: ${data.msg ?? res.statusText}`,
    );
  }
...
export async function createLarkFile(
  type: LarkFileType,
  title: string,
  ...
  try {
    switch (type) {
      case "docx": {
        ...
        return {
          documentId,
          url: `https://${process.env.LARK_WORKSPACE_DOMAIN}/docx/${documentId}`,
          type,
        };
      }
      case "sheet": {
        ...
        return {
          documentId: data.spreadsheet.spreadsheet_token,
      ...
      case "bitable": {
        const data = await larkFetch(
          "/bitable/v1/apps",
          { name: title, ...folderField },
          app.key,
        );
        return { documentId: data.app.app_token, url: data.app.url, type };
      }
      case "folder": {
        const folderTokenForCreate =
          folderToken || (await getMySpaceRootFolderToken(app.key));
        const data = await larkFetch(
          "/drive/v1/files/create_folder",
          { name: title, folder_token: folderTokenForCreate },
          app.key,
        );
    ...
    throw new Error(
      `Không tạo được ${label}: ${err instanceof Error ? err.message : "lỗi không rõ"}`,
    );
  }
}
...
export async function listFolderChildren(
  folderToken: string,
  appKey?: string,
): Promise<LarkFolderEntry[]> {
  const token = await getTenantAccessToken(appKey);
  const res = await fetch(
    `${LARK_API_BASE}/drive/v1/files?folder_token=${folderToken}&page_size=200`,
    {
      headers: { Authorization: `Bearer ${token}` },
      cache: "no-store",
    },
...
export async function listTenantContacts(
  ...
  for (let i = 0; i < userIds.length; i += 50) {
    ...
    const res = await fetch(
      `${LARK_API_BASE}/contact/v3/users/batch?${query}&user_id_type=open_id`,
      {
        headers: { Authorization: `Bearer ${token}` },
        cache: "no-store",
      },
...
export async function getAppRootFolderToken(appKey?: string): Promise<string> {
  const app = getLarkAppConfig(appKey);
  return (
    app.docFolderToken?.trim() || (await getMySpaceRootFolderToken(app.key))
  );
}
...
export async function moveLarkFile(
  ...
  const token = await getTenantAccessToken(appKey);
  const res = await fetch(
    `${LARK_API_BASE}/drive/v1/files/${documentId}/move`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function deleteLarkFile(
  documentId: string,
  type: LarkFileType,
  appKey?: string,
): Promise<void> {
  const token = await getTenantAccessToken(appKey);
  const res = await fetch(
    `${LARK_API_BASE}/drive/v1/files/${documentId}?type=${type}`,
    {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
      cache: "no-store",
...
export async function transferLarkFileOwner(
  ...
  if (!res.ok || data.code !== 0) {
    throw new Error(
      `Không chuyển được quyền sở hữu cho ${email}: ${data.msg ?? res.statusText}`,
    );
  }
}
